...
class ProductCategorySeeder extends Seeder
{
     ...
     */
    private static function productCategoryJson(): ?array
    {
        $productCategoryJson = collect(
            json_decode(
                '[
                    {
                        "product_id": 1,
        ...
        foreach ($productCategoryJson as $product) {
            $categories[$product->category_id] = $product->category_title;
        }
    ...
    {
        $categories = self::productCategoryJson();

        if ($categories && is_countable($categories)) {
            foreach ($categories as $key => $category) {
                if (!ProductCategory::query()->find($key)) {
